...
"none"===c?(this.doingTransition=!1,setTimeout(function(){b.removeClass("active");d.addClass("active");a(e).addClass("active")},50)):this.runTransition(c,b,d,g,e)},updateViewHistory:function(a,d,b,e){var g=this.views[a.prop("id")];g||(g=this.views[a.prop("id")]=[]);1<=g.length&&g[g.length-1].target===d||(this.pushHistory(d.id,d.id,b,e.replace(d.id,"").replace("#","")),this.views[a.prop("id")].push({target:d,transition:b}))},launch:function(){if(!1===this.hasLaunched||this.launchCompleted)this.hasLaunched=
!0;else{if(this.isLaunching)return!0;this.isLaunching=!0;this.blockPageBounce();var c=this,d=a.create("div",{id:"afui_mask",className:"ui-loader",html:"<span class='ui-icon ui-icon-loading spin'></span><h1>Loading Content</h1>"}).css({"z-index":2E4,display:"none"});document.body.appendChild(d.get(0));d=a(".view[data-default='true']");if(0===d.length)d=a(".view").eq(0);else throw"You need to create a view";d.addClass("active");this.views[d.prop("id")]=[];var e=window.location.hash,f=0===d.find(".panel[data-selected='true']").length?
d.find(".panel").eq(0):d.find(".panel[data-selected='true']");f.addClass("active");this.activeDiv=f.get(0);this.views[d.prop("id")].push({target:f.get(0),transition:this.transitionType});this.defaultPanel=f.get(0);this.loadContentData(f.get(0),d,!1,!0);this.updateHash(f.get(0).id);this.loadDefaultHash&&setTimeout(function(){this.loadContent(e,!1,0,"none")}.bind(this));this.enableTabBar();a(document).on("click","a",function(a){c.useInternalRouting&&g(a,a.currentTarget)});a(document).on("click",function(d){for(var e=
b.length,g=a(d.target),f=0;f<e;f++){var h=b[f],m=g.closest(h.sel);0<m.length&&h.cb.call(c,m.get(0),d)}});a(document).on("click",".backButton, [data-back]",c.goBack.bind(c));d=a("[data-include]");if(0===d.length)this.launchCompleted=!0,a(document).trigger("afui:ready");else{var h=[];d.each(function(){var c=this.getAttribute("data-include"),d=a(this);h.push(a.get(c,function(a){d.append(a)}))});a.when.apply(a,h).then(function(){this.launchCompleted=!0;a(document).trigger("afui:ready")}).fail(function(){this.launchCompleted=
...
(function(a){function f(){return this}var h=null,b,g={push:function(b,c,d){b=a(b).closest(".view").children().filter(":not(nav):not(aside)");d=d||h;for(var g=0;g<b.length;g++){var f=a(b[g]).show().animation();c?f.remove("slide-"+d+"-out").reverse():f.keep();f.run("slide-"+d+"-out")}},cover:function(){},reveal:function(a,b){return this.push(a,b)}};f.prototype={defaultTransition:"slide",defaultAnimation:"cover",isTransitioning:!1,autoHide:function(a){a.preventDefault();this.hide()},checkViewToClose:function(a){this.autoHide(a)},
